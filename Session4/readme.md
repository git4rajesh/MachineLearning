...
Factors that will be considered when we design a Model are given in the sequence of their relevance:

## How many layers :

- Consider Global Receptive Field and since the input size is 28 * 28 , make sure we reach that in our global receptive field at our last convolution layer.
- Also keep convolving till we have an input of size n * n Size where we have more number of Pixels that get convolved uniformly say 9 times ( using 3 * 3 Kernel)
...
- In our case we do till we reach an input channel size of 7 * 7 .
  ...
  - Since the input size is 28 * 28 , make sure we reach that in our global receptive field at our last convolution layer.
